 ...
 *
 * Backed by Device Protected Storage so the receiver can read it in the
 * Direct Boot phase (LOCKED_BOOT_COMPLETED, before first unlock) and start
 * the VPN immediately after a reboot. Trade-off: DE storage is not
 * credential-encrypted; the auth key already lives unencrypted on the
 * device either way.
 */
@Singleton
class SettingsRepository @Inject constructor(
    ...
    private val deContext: Context = context.createDeviceProtectedStorageContext().also {
        // One-time move of prefs written by older builds into DE storage
        // (no-op when the source file does not exist).
        it.moveSharedPreferencesFrom(context, PREFS_NAME)
    }

    private val prefs: SharedPreferences
        get() = deContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
