...
Branch history: originally `claude/android-client-changes-omjrcc` (PR #1, base
v0.9.0). That branch was merged into `claude/mqvpn-apk-rebuild-87to7e` on top of
upstream **v0.11.1**, which also carries per-provider live throughput
(ProviderDirectory/SpeedTracker/ThroughputChart, rename-by-tap) and a Direct
Boot port from `feat/patch` (directBootAware service/receiver,
LOCKED_BOOT_COMPLETED, device-protected prefs with one-time migration). The
early custom hybrid plumbing was reconciled in favor of upstream's official
v0.11 hybrid lane; the custom `configSetHybridLimits` extension was kept.

## What the custom feature set adds (originally on top of v0.9.0)
